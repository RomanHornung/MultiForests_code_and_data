...
void Forest::run(bool verbose, bool compute_oob_error) {
  ...
  if (prediction_mode) {
    ...
    if (importance_mode == IMP_PERM_BREIMAN || importance_mode == IMP_PERM_LIAW || importance_mode == IMP_PERM_RAW || importance_mode == MUWIMP_BOTH || importance_mode == MUWIMP_MULTIWAY || importance_mode == MUWIMP_BINARY) {
      if (verbose && verbose_out) {
